...
def title_to_filename_stub(title: str) -> Path:
  ...
  * The first seven (lower-case) characters of the MD5 hash of the original
  ...
  for aPart in parts :
    ...
    newPart = underscoreRunsRegExp.sub('_', newPart)
...
def deserialise_tid(
  filename: Path, include_text: bool = True
) -> dict[str, str]:
  """
...
def serialise_json_plus_text(
  tiddler: dict[str, str], filename: Path
) -> None:
  """
...
def deserialise_json_plus_text(
  filename: Path, include_text: bool = True
) -> dict[str, str]:
  """
